...
class Solution {
    ...
    public void setData(int n) {
        // OPTIMIZATION 1: Arrays are now strictly size 'n' instead of 'n+1'
        parent = new int[n];
        size = new int[n];

        // Loop changed to strictly less than 'n'
        for(int i = 0; i < n; i++) {
            parent[i] = i;
            size[i] = 1;
        }
    ...
    public int SolutionUsingUnionDisjointSet(int[][] grid) {
        ...
        // OPTIMIZATION 2: Only check Down {1,0} and Right {0,1}
        // Since we iterate top-to-bottom, left-to-right, Up and Left are already handled
        ...
        for(int r = 0; r < row_size; r++) {
            for(int c = 0; c < col_size; c++) {
                if(grid[r][c] == 1) { 
                    ...
                    // Continually update the max size encountered
                    max = Math.max(max, findSize(r * col_size + c));
                }
            }
        }
